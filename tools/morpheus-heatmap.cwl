...
arguments:
 - valueFrom: >
      if [ ! -z "$(inputs.read_counts_gct)" ] && [ -s "$(inputs.read_counts_gct.path)" ]; then
         run_morpheus.R --gct "$(inputs.read_counts_gct.path)" $(if [ ! -z "$(inputs.output_prefix)" ]; then echo "--output $(inputs.output_prefix)"; fi);
      else
