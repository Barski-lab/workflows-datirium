...
# Docker paths
workflow_file <- "/usr/local/bin/functions/atac/workflow.R"
...
if (file.exists(workflow_file)) {
  ...
  if (file.exists(verify_file)) {
    ...
    output_prefix <- if (!is.null(args$output_prefix)) args$output_prefix else 
                     if (!is.null(args$output)) args$output else "atac"
    
    # Verify all required outputs were created
    message("Verifying all required outputs were created...")
    verify_workflow_outputs("atac_advanced", output_prefix, fail_on_missing = FALSE)
  } else {
  ...
  # Try relative path
  relative_path <- "functions/atac/workflow.R"
  if (file.exists(relative_path)) {
    ...
    system("find /usr/local -name workflow.R | grep atac/", intern = FALSE)
    stop("Could not find workflow.R file. Please verify your installation.")
  }
}

message("ATAC-seq analysis completed.")
