...
run_workflow <- function(args) {
  ...
  log_message(paste("Loading ATAC-seq object data from", args$dsq_obj_data))
  ...
  if (!is.null(expDataDf)) {
    ...
    log_warning("expDataDf is NULL - will need to create from ATAC-seq object")
    # Create expression data from the ATAC-seq object if missing
  ...
  if (is.null(dds)) {
    stop("ATAC-seq dataset is missing from step 1 data")
  }
  
  # Extract metadata from ATAC-seq object
  ...
  log_message(paste("Available factors in ATAC-seq dataset:", paste(factor_names, collapse = ", ")))
  ...
  if (exists("factors", where = step1_data)) {
    for (factor_name in step1_data$factors) {
      if (factor_name %in% factor_names) {
        ...
        log_warning(paste("Factor", factor_name, "from step 1 not found in ATAC-seq dataset"))
      }
    }
    log_message(paste("Using reference levels:", paste(names(reference_levels), reference_levels, sep = "=", collapse = ", ")))
  } else {
    log_warning("No factors list found in step 1 data - using factor information from ATAC-seq dataset")
  ...
  # Get normalized counts from ATAC-seq
  ...
  for (contrast_name in names(results_list)) {
    ...
    ma_plot <- function() {
      ATAC-seq::plotMA(contrast_results, main=paste("MA Plot -", contrast_name))
    }
