...
#' Rebuild ATAC-seq object with specific factor reference levels
#'
#' @param dds ATAC-seq data object
#' @param factor_ref_list Named list of factors and their reference levels
#' @return Updated ATAC-seq data object
#' @export
...
apply_batch_correction <- function(dds, col_data, batch_correction_method) {
  ...
  if (batch_correction_method == "model" && "batch" %in% colnames(col_data)) {
    # Case: After ATAC-seq, apply rlog transformation and remove batch effects using limma
...
#'
#' @param col_data Column data from ATAC-seq dataset
#' @param design_formula Design formula from ATAC-seq dataset
...
verify_step_consistency <- function(col_data, design_formula, args) {
  ...
  if (args$batchcorrection != "none") {
    ...
    if (!"batch" %in% colnames(col_data)) {
      log_warning("Batch correction requested but 'batch' column not found in ATAC-seq data")
