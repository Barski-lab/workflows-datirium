...
#' Process a specific contrast and obtain ATAC-seq results
#'
...
#' @return ATAC-seq results object
#' @export
get_contrast_res <- function(step1_data, contrast_row, args) {
  # Extract the ATAC-seq dataset from step1_data
...
#'
#' @param dds_base ATAC-seq data object
#' @param contrast_row Contrast information row
#' @param args Command-line arguments
...
process_main_effect_contrast <- function(dds_base, contrast_row, args, lfcThreshold, altHypothesis) {
  # Get column names from ATAC-seq object to identify available factors
  ...
  log_message("ATAC-seq results for main effect contrast obtained successfully")
  return(res)
}
...
process_interaction_contrast <- function(dds_base, contrast_row, args, lfcThreshold, altHypothesis) {
  ...
  if (grepl(":", contrast_row$contrast, fixed = TRUE)) {
    log_message("Processing ATAC-seq formula-style interaction contrast")
    # This is likely a ATAC-seq formula-style interaction term
  ...
  log_message("ATAC-seq results for interaction contrast obtained successfully")
  return(res)
}

#' Add ATAC-seq results as metadata columns to expression data
#'
#' @param expression_data_df Expression data frame
#' @param deseq_result ATAC-seq results object
...
add_metadata_to_results <- function(expression_data_df, deseq_result, contrast_name) {
  log_message(paste("Adding ATAC-seq results for contrast:", contrast_name))
  ...
  if (length(missing_columns) > 0) {
    log_warning(paste("Missing expected columns in ATAC-seq results:", paste(missing_columns, collapse=", ")))
  ...
  log_message(paste("ATAC-seq results dimensions:", nrow(res_df), "x", ncol(res_df)))
  ...
  if (length(selected_columns) == 0) {
    log_warning("No target columns found in ATAC-seq results - using all columns")
  ...
  log_message("ATAC-seq results prepared for merging")
  ...
  if (!all(rownames(expression_data_df) == rownames(res_df))) {
    ...
    if (length(common_rows) == 0) {
      stop("No common row names between expression data and ATAC-seq results")
    }
  ...
  log_message(paste("Successfully merged ATAC-seq results for contrast", contrast_name))
