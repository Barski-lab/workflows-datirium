#!/usr/bin/env Rscript
#
# Workflow functions for ATAC-seq LRT Step 2
#
...
#' Main workflow function for ATAC-seq LRT Step 2
#'
...
initialize_environment <- function() {
  ...
  # Source ATAC-seq LRT Step 2 specific functions
  source_with_fallback("functions/atac_lrt_step_2/cli_args.R", "/usr/local/bin/functions/atac_lrt_step_2/cli_args.R")
  source_with_fallback("functions/atac_lrt_step_2/data_processing.R", "/usr/local/bin/functions/atac_lrt_step_2/data_processing.R")
  source_with_fallback("functions/atac_lrt_step_2/contrast_analysis.R", "/usr/local/bin/functions/atac_lrt_step_2/contrast_analysis.R")
  ...
  log_message("Environment initialized for ATAC-seq LRT Step 2 analysis")
}


run_workflow <- function(args) {
  log_message("Starting ATAC-seq LRT Step 2 workflow")
  
  # Load input data from Step 1
  log_message(paste("Loading ATAC-seq object data from", args$atac_obj_data))
  step1_data <- readRDS(args$atac_obj_data)
  ...
  log_message(paste("Available factors in ATAC-seq DESeq2 dataset:", paste(factor_names, collapse = ", ")))
  ...
  if (exists("factors", where = step1_data)) {
    for (factor_name in step1_data$factors) {
      if (factor_name %in% factor_names) {
        ...
        log_warning(paste("Factor", factor_name, "from step 1 not found in ATAC-seq DESeq2 dataset"))
      }
    }
    log_message(paste("Using reference levels:", paste(names(reference_levels), reference_levels, sep = "=", collapse = ", ")))
  } else {
    log_warning("No factors list found in step 1 data - using factor information from ATAC-seq DESeq2 dataset")
  ...
  if (args$batchcorrection != "none") {
    ...
    log_message("Batch correction completed for ATAC-seq data")
  } else {
    log_message("No batch correction applied to normalized ATAC-seq counts")
  }
  
  # Create MDS plot for sample visualization
  log_message("Creating MDS plot for ATAC-seq sample visualization")
  with_error_handling({
    create_mds_plot(
  ...
  for (i in 1:nrow(contrast_df)) {
    ...
    if (is.null(contrast_res)) {
      log_warning(paste("Failed to process ATAC-seq contrast:", contrast_name))
      next
    }
    ...
    if (!is.null(updated_expDataDf)) {
      ...
      log_message(paste("Added results for ATAC-seq contrast", contrast_name, "to expression data"))
    }
  ...
  if (!is.null(args$output)) {
    output_file <- paste0(args$output, "_atac_step2_results.rds")
    log_message(paste("Saving ATAC-seq results to", output_file))
    saveRDS(final_results, file = output_file)
  }
  
  # Export reports
  log_message("Exporting ATAC-seq reports and visualizations", "STEP")
  
  # Export ATAC-seq results table with CWL-expected filename
  atac_results_file <- paste0(args$output, "_atac_results_table.tsv")
  write_atac_results(expDataDf, atac_results_file)
  ...
  for (contrast_name in names(results_list)) {
    ...
    if (requireNamespace("ggplot2", quietly = TRUE)) {
      ...
      volcano_plot <- ggplot2::ggplot(
        ...
        ggplot2::labs(
          title = paste("ATAC-seq Volcano Plot -", contrast_name),
          x = "log2 Fold Change",
          y = "-log10(p-value)"
        ) +
      ...
      log_message(paste("Created ATAC-seq volcano plot for", contrast_name), "INFO")
    }
  }
