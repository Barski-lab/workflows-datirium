#!/usr/bin/env Rscript
#
# Data processing functions for DESeq/ATAC-seq differential accessibility analysis
#
# This file contains functions for loading, preprocessing, and filtering
# accessibility data for the ATAC-seq analysis workflow.
#
# Version: 0.1.0
...
#' Filter accessibility data by RPKM threshold
#'
#' @param accessibility_df Data frame containing accessibility data
#' @param n RPKM threshold value
#' @return Filtered data frame
filter_rpkm <- function(accessibility_df, n) {
  log_message(paste("Filtering accessibility data with RPKM cutoff:", n))
  
  if (!requireNamespace("dplyr", quietly = TRUE)) {
    stop("Package 'dplyr' is required for RPKM filtering")
  }
  
  filtered_df <- accessibility_df %>%
    dplyr::filter(dplyr::if_any(dplyr::contains("Rpkm"), ~. > n))
  
  log_message(paste("Filtered", nrow(accessibility_df) - nrow(filtered_df), "rows by RPKM cutoff"))
...
apply_test_mode <- function(accessibility_df, max_genes = 1000) {
  log_message(paste("Applying test mode - reducing data to", max_genes, "genes for faster processing"))
  
  original_rows <- nrow(accessibility_df)
  ...
  # Sort by average accessibility across all samples and take top genes
  read_cols <- grep("TotalReads|_treated|_untreated", colnames(accessibility_df), value = TRUE)
  if (length(read_cols) > 0) {
    accessibility_df$avg_accessibility <- rowMeans(accessibility_df[read_cols], na.rm = TRUE)
    accessibility_df <- accessibility_df[order(accessibility_df$avg_accessibility, decreasing = TRUE), ]
    accessibility_df <- accessibility_df[1:min(max_genes, nrow(accessibility_df)), ]
    accessibility_df$avg_accessibility <- NULL
  } else {
    # Fallback: just take the first N genes
    accessibility_df <- accessibility_df[1:min(max_genes, nrow(accessibility_df)), ]
  }
  
  log_message(paste("Test mode: Reduced from", original_rows, "to", nrow(accessibility_df), "genes"))
  
  return(accessibility_df)
}

#' Load accessibility data from isoform files and merge them
#'
...
#' @return List containing merged accessibility data and metadata
...
#' @param atac_results Results from DESeq/ATAC-seq analysis
#' @param output_file Path to the output markdown file
#' @return None
generate_md <- function(batchcorrection, batchfile, atac_results, output_file) {
  # Initialize the markdown content
  md_lines <- c("# ATAC-seq Summary\n")
  ...
  if (!is.null(atac_results)) {
    # Extract total genes with non-zero read count
    total_genes <- sum(!is.na(atac_results$baseMean) & atac_results$baseMean > 0)

    # Get thresholds from atac_results metadata
    metadata_res <- metadata(atac_results)
    ...
    # Extract counts
    res_nonzero <- atac_results[!is.na(atac_results$baseMean) & atac_results$baseMean > 0,]
    ...
    md_lines <- c(md_lines,
      "\n## Notes\n",
      "1. Outliers are genes with high Cook's distance (see [cooksCutoff](https://www.bioconductor.org/packages/release/bioc/vignettes/ATAC-seq/inst/doc/ATAC-seq.html#outlier)).",
      "2. Low counts are genes filtered out due to low mean counts (see [independentFiltering](https://www.bioconductor.org/packages/release/bioc/vignettes/ATAC-seq/inst/doc/ATAC-seq.html#independent-filtering-of-results))."
    )
  }
