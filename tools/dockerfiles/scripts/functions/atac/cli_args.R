#!/usr/bin/env Rscript
#
# Command-line argument handling for DESeq/ATAC-seq differential accessibility analysis
#
# This file contains functions for parsing and validating command-line arguments
# for the main ATAC-seq analysis workflow.
#
# Version: 0.1.0
...
assert_args <- function(args) {
  ...
  if (is.null(args$untreated_sample_names) | is.null(args$treated_sample_names)) {
    log_message("--untreated_sample_names or --treated_sample_names were not set, using default values based on accessibility file names")
  ...
  if (length(args$treated_files) == 1 || length(args$untreated_files) == 1) {
    log_warning("Only one file in a group. ATAC-seq requires at least two replicates for accurate analysis.")
...
#' Parse command line arguments for ATAC-seq analysis
#'
#' @return Parsed and validated argument list
get_args <- function() {
  parser <- ArgumentParser(description = "Run DESeq/ATAC-seq for untreated-vs-treated groups (condition-1-vs-condition-2)")
  
  # Input file parameters
  parser$add_argument(
    "-u", "--untreated_files",
    help = "Untreated (condition 1) CSV/TSV isoforms accessibility files",
    type = "character",
    required = TRUE,
    nargs = "+"
  )
  parser$add_argument(
    "-t", "--treated_files",
    help = "Treated (condition 2) CSV/TSV isoforms accessibility files",
    type = "character",
    required = TRUE,
    nargs = "+"
  )
  parser$add_argument(
    ...
    help = "Unique aliases for untreated (condition 1) accessibility files. Default: basenames of -u without extensions",
    type = "character",
    nargs = "*"
  )
  parser$add_argument(
    "-ta", "--treated_sample_names",
    help = "Unique aliases for treated (condition 2) accessibility files. Default: basenames of -t without extensions",
    type = "character",
    nargs = "*"
  )
  ...
  parser$add_argument(
    ...
    help = paste(
      ...
      "- 'combatseq' applies ComBat_seq at the beginning of the analysis, removing batch effects from the design formula before differential accessibility analysis.",
      "- 'model' applies removeBatchEffect from the limma package after differential accessibility analysis, incorporating batch effects into the model during DE analysis.",
      "- Default: none"
    ),
    type = "character",
  ...
  parser$add_argument(
    "--regulation",
    help = paste(
      "Direction of differential accessibility comparison. β is the log2 fold change.",
  ...
  parser$add_argument(
    "--scaling_type",
    help = paste(
      "Specifies the type of scaling to be applied to the accessibility data.",
  ...
  parser$add_argument(
    "-o", "--output_prefix",
    help = "Output prefix. Default: atac",
    type = "character",
    default = "./atac"
  )
  parser$add_argument(
