...
message("Starting ATAC-seq LRT Step 2 Analysis")
message("Working directory:", getwd())

# Docker paths
workflow_file <- "/usr/local/bin/functions/atac_lrt_step_2/workflow.R"
...
if (file.exists(workflow_file)) {
  ...
  if (file.exists(verify_file)) {
    ...
    output_prefix <- if (!is.null(args$output_prefix)) args$output_prefix else 
                     if (!is.null(args$output)) args$output else "atac-lrt-step-2"
    
    # Verify all required outputs were created
    message("Verifying all required outputs were created...")
    verify_workflow_outputs("atac_lrt_step2", output_prefix, fail_on_missing = FALSE)
  } else {
  ...
  # Try relative path
  relative_path <- "functions/atac_lrt_step_2/workflow.R"
  if (file.exists(relative_path)) {
    ...
    system("find /usr/local -name workflow.R | grep atac_lrt_step_2/", intern = FALSE)
    stop("Could not find workflow.R file. Please verify your installation.")
  }
}

message("ATAC-seq LRT Step 2 analysis completed.")