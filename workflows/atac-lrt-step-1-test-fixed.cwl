...
inputs:
  ...
  test_peak_files:
    ...
    label: "ATAC-seq experiments"
    doc: "CSV/TSV input files grouped by isoforms"
  #    'sd:upstreamSource': "atacseq_experiment/rpkm_isoforms"
  #    'sd:localLabel': true

  peak_file_names:
    type: string[]
    label: "ATAC-seq experiments"
    doc: "Aliases for ATAC-seq experiments. The same aliases should be used in metadata file"
  #    'sd:upstreamSource': "atacseq_experiment/alias"
...
doc: |
  ...
  **Technical notes**
  ...
  2. **Metadata File:** The metadata file describes relations between compared experiments. For example:
     ...
     DH5,day7,KO
     ```
     where `time`, `condition`, `day5`, `day7`, `WT`, `KO` should be single words (without spaces), and `DH1`, `DH2`, `DH3`, `DH4`, `DH5` correspond to the experiment aliases set in **ATAC-seq experiments** input.
  ...
  4. **Batch Correction:** If batch correction is required, provide the `batch_file` input. This file should be a headerless TSV/CSV file where the first column contains sample names matching `peak_file_names`, and the second column contains the batch group name.
