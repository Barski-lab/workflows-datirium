...
get_args <- function() {
  ...
  tryCatch({
    ...
    if (exists("cli_helpers") && is.environment(cli_helpers)) {
      ...
      args$untreated_files <- parse_multi_args(all_args, c("-u", "--untreated_files"))
      args$treated_files <- parse_multi_args(all_args, c("-t", "--treated_files"))
      args$untreated_sample_names <- parse_multi_args(all_args, c("-ua", "--untreated_sample_names"))
      args$treated_sample_names <- parse_multi_args(all_args, c("-ta", "--treated_sample_names"))
      
      # Single-value arguments with defaults
      args$untreated_name <- parse_single_arg(all_args, c("-un", "--untreated_name"), "untreated")
      args$treated_name <- parse_single_arg(all_args, c("-tn", "--treated_name"), "treated")
      args$batch_file <- parse_single_arg(all_args, c("-bf", "--batch_file"))
      args$batchcorrection <- parse_single_arg(all_args, c("--batchcorrection"), "none")
      args$regulation <- parse_single_arg(all_args, c("--regulation"), "both")
      args$cluster_method <- parse_single_arg(all_args, c("--cluster_method"), "none")
      args$scaling_type <- parse_single_arg(all_args, c("--scaling_type"), "zscore")
      args$row_distance <- parse_single_arg(all_args, c("--row_distance"), "cosangle")
      args$column_distance <- parse_single_arg(all_args, c("--column_distance"), "euclid")
      args$output_prefix <- parse_single_arg(all_args, c("-o", "--output_prefix"), "./deseq")
      
      # Numeric arguments
      args$fdr <- as.numeric(parse_single_arg(all_args, c("--fdr"), "0.1"))
      args$lfcthreshold <- as.numeric(parse_single_arg(all_args, c("--lfcthreshold"), "0.59"))
      args$k_hopach <- as.integer(parse_single_arg(all_args, c("--k"), "3"))
      args$kmax_hopach <- as.integer(parse_single_arg(all_args, c("--kmax"), "5"))
      args$threads <- as.integer(parse_single_arg(all_args, c("-p", "--threads"), "1"))
      args$digits <- as.integer(parse_single_arg(all_args, c("-d", "--digits"), "3"))
      rpkm_val <- parse_single_arg(all_args, c("--rpkm_cutoff"))
      args$rpkm_cutoff <- if (is.null(rpkm_val)) NULL else as.integer(rpkm_val)
      
      # Boolean flags
      args$use_lfc_thresh <- parse_bool_arg(all_args, c("--use_lfc_thresh"))
      args$test_mode <- parse_bool_arg(all_args, c("--test_mode"))
      
      message("Directly extracted required argument: untreated_files = ", paste(args$untreated_files, collapse=", "))
      message("Directly extracted required argument: treated_files = ", paste(args$treated_files, collapse=", "))
      if (length(args$untreated_sample_names) > 0) {
        message("Extracted optional array argument: untreated_sample_names = ", paste(args$untreated_sample_names, collapse=", "))
      }
      if (length(args$treated_sample_names) > 0) {
        message("Extracted optional array argument: treated_sample_names = ", paste(args$treated_sample_names, collapse=", "))
      }
    }
  ...
  # Debug: Check what args is before calling assert_args
  message("DEBUG: Before assert_args - args class = ", class(args))
  message("DEBUG: Before assert_args - args type = ", typeof(args))
  message("DEBUG: Before assert_args - is.function(args) = ", is.function(args))
  message("DEBUG: Before assert_args - is.list(args) = ", is.list(args))
  
  # Validate arguments and set defaults
  parsed_args <- assert_args(args)
  
  # Convert numeric values
  for (arg_name in c("fdr", "lfcthreshold", "threads")) {
    if (!is.null(parsed_args[[arg_name]]) && is.character(parsed_args[[arg_name]])) {
      if (grepl("^[0-9.]+$", parsed_args[[arg_name]])) {
        parsed_args[[arg_name]] <- as.numeric(parsed_args[[arg_name]])
      }
    }
  }
