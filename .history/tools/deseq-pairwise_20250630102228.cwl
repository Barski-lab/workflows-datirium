...
- class: InitialWorkDirRequirement
  listing:
    - entryname: "functions/deseq/cli_args.R"
      entry: $(inputs.override_cli)


hints:
- class: DockerRequirement
  dockerPull: "biowardrobe2/scidap-deseq:v0.0.71"
...
arguments:
  - |
    set -euo pipefail
    # copy patched CLI parser into the location sourced by workflow.R
    mkdir -p /usr/local/bin/functions/deseq
    cp functions/deseq/cli_args.R /usr/local/bin/functions/deseq/cli_args.R
    # hand over to the original entry-point, forwarding all CWL-generated args
    exec /usr/bin/Rscript /usr/local/bin/run_deseq_pairwise.R "$@"
